...
function mongoc_collection:command_simple(command, reply, read_prefs)
	...
	return b, ffi_string(er.message)
end
...
function mongoc_collection:count(query, skip, limit, flags, read_prefs)
	...
	return v, ffi_string(er.message)
end
...
function mongoc_collection:find_indexes()
	...
	if ptr then
		...
		return nil, ffi_string(er.message)
	end
end
...
function mongoc_collection:find_and_modify(query, sort, update, fields, _remove, upsert, _new)
	local the_bson = bson.new() 
	local er = ffi_new('bson_error_t')
	local b = collection_find_and_modify(self.ptr, query, sort, update, fields, _remove, upsert, _new, the_bson.ptr, er)
	return b, the_bson:read_values(), ffi_string(er.message)
end
