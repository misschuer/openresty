...
function mongo_database_wrap:getCollection(name)
	local collection_wrap = mongo_database_wrap[name]	
	if not collection_wrap then
		...
		mongo_database_wrap[name] = collection_wrap
	end
...
function mongo_collection_wrap:insert(values)
	return self.collection:insert(getBsonPtrByTable(values))
end

--@wheres: 	查询的条件
function mongo_collection_wrap:delete(wheres)
	return self.collection:remove(getBsonPtrByTable(wheres))
end
...
function mongo_collection_wrap:update(wheres, values, upsert, multi )
	...
	return self.collection:update(getBsonPtrByTable(wheres), getBsonPtrByTable(values), flags)
end
...
function mongo_collection_wrap:find(wheres, fields, limit, skip)
	if mongo_collection_wrap.last_use_cursor then
		mongo_collection_wrap.last_use_cursor:destroy()	
		mongo_collection_wrap.last_use_cursor = nil
	end
	...
	if cursor ~= nil then
		...
		mongo_collection_wrap.last_use_cursor = cursor
	end
	return cursor_wrap
end
...
function mongo_collection_wrap:findOne(wheres, fields)
	local cursor = self:find(wheres, fields, 1)
	return cursor:next()
end

--@wheres: 	查询的条件
--@fields: 	返回的字段 
function mongo_collection_wrap:findAndModify(wheres, update, new, upsert)
	local cursor_wrap = nil
	local b, reply,re = self.collection:find_and_modify(getBsonPtrByTable(wheres), nil, getBsonPtrByTable(update), nil, false, upsert or false, new or false)
	return b, reply, re
end
